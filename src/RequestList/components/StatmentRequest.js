...
function StatmentRequest(props) {
  ...
  if (props.json === true) {
    ...
    content = <div className={globalStyle.content}>
          ...
          <Typography className={globalStyle.line}>Org name: {props.request.orgName}</Typography>
          <Typography className={globalStyle.line}>From date: {props.request.fromDate}</Typography>
          <Typography className={globalStyle.line}>To date: {props.request.toDate}</Typography>
          {props.request.accounts.map((item, index) => (
            <Typography className={globalStyle.line} key={index}>Bank name {index + 1}: {item.bankName}</Typography>
          ))}
        </div>
  }
